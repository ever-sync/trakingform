...
import { Loader2, Plus, Save, Trash2, Play } from 'lucide-react'
...
function normalizeRule(raw: Partial<RuleItem>): RuleItem {
  const assignmentRaw = (raw.assignment && typeof raw.assignment === 'object' && !Array.isArray(raw.assignment))
  ...
  return {
    id: String(raw.id ?? `tmp_${Date.now()}`),
...
export function RulesBuilder() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([rulesData, membersData]) => {
        ...
        setRules(rawRules.map((item: Partial<RuleItem>) => normalizeRule(item)))

        const rawMembers = Array.isArray(membersData.members) ? membersData.members : []
        setMembers(rawMembers.map((row) => ({
  ...
  async function saveRule(rule: RuleItem) {
    ...
    try {
      ...
      setRules((prev) => prev.map((item) => (item.id === rule.id ? normalizeRule(saved) : item)))
  ...
  async function testAssignment() {
    ...
    try {
      ...
      if (payload.assigned) {
        toast.success(`Atribuido pela regra: ${payload.ruleName ?? 'N/A'}`)
      } else {
              ...
              <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                ...
                <div className="space-y-1.5">
                  ...
                  <Input
                    ...
                    value={rule.priority}
                    onChange={(event) =>
                      setRules((prev) => prev.map((item) => (item.id === rule.id ? { ...item, priority: Number(event.target.value) || 0 } : item)))
                    }
                  />
                </div>
              </div>
                ...
                {rule.conditions.length === 0 ? (
                  ...
                  <div className="space-y-2">
                    {rule.conditions.map((condition, conditionIndex) => (
                      <div key={`${rule.id}_${conditionIndex}`} className="grid grid-cols-1 gap-2 md:grid-cols-[1fr_1fr_1fr_auto]">
                        <select
                        ...
                        <select
                          ...
                          onChange={(event) => updateCondition(rule.id, conditionIndex, { operator: event.target.value as RoutingConditionItem['operator'] })}
                        >
                  ...
                  {rule.assignment.mode === 'fixed_user' ? (
                    <select
                      ...
                      onChange={(event) =>
                        setRules((prev) => prev.map((item) => (
                          item.id === rule.id
                            ? { ...item, assignment: { ...item.assignment, user_id: event.target.value || undefined } }
                            : item
                        )))
                      }
                    >
